	...
	w2toolbar.prototype = {
		...
		insert: function (id, options) {
			...
			for (var o in options) {
				// checks
				if (typeof options[o].type == 'undefined') {
					$.error('The parameter "type" is required but not supplied in w2toolbar.add() method.');
					return;
				}
				if ($.inArray(String(options[o].type), ['button', 'check', 'radio', 'drop', 'menu', 'break', 'html', 'spacer']) == -1) {
				...
				var unique = true;
				for (var i = 0; i < this.items.length; i++) { if (this.items[i].id == options[o].id) { unique = false; return; } }
				if (!unique) {
					$.error('The parameter "id" is not unique within the current toolbar.');
					return;
				}
				if (!w2utils.isAlphaNumeric(options[o].id)) {
					$.error('The parameter "id" must be alpha-numeric + "-_".');
					return;
				}
				// add item
				var it = $.extend({}, w2toolbar.prototype.item, options[o]);
		...
		render: function (box) {
			...
			// render all buttons
			$(this.box).html('');
			var html = '<div id="'+ this.name +'_toolbar" class="w2ui-toolbar">'+
			...
			for (var i = 0; i < this.items.length; i++) {
				...
				if (it.disabled) { addStyle += 'opacity: 0.2; -moz-opacity: 0.2; -webkit-opacity: 0.2; filter:alpha(opacity=20);'; }
				if (it.type == 'spacer') {
			...
			if ($('#w2ui-global-drop').length == 0) $('body').append('<div id="w2ui-global-drop"></div>');
			// event after
		...
		refresh: function (id) {
			...
			if (jq_el.length == 0) {
				...
				if (it.disabled) { jq_el.css({ 'opacity': '0.2', '-moz-opacity': '0.2', '-webkit-opacity': '0.2', 'filter': 'alpha(opacity=20)' }); }
		...
		getMenuHTML: function (item) { 
			...
			for (var f = 0; f < item.items.length; f++) { 
				...
				menu_html += "<tr style=\"cursor: default;\" "+
					...
					"			  obj.doClick('"+ item.id +"', '"+ w2utils.stripTags(tmp[2]) +"', '"+ f +"', event);\">"+
				...
				case 'drop':
					html +=  '<table cellpadding="0" cellspacing="0" title="'+ item.hint +'" class="w2ui-tab0 '+ (item.checked ? 'checked' : '') +'" '+
							 ...
							 '  <tr>'+
									...
									(((item.type == 'drop' || item.type == 'menu') && item.arrow !== false) ? '<td class="w2ui-tb-down" nowrap></td>' : '') +
							 '  </tr></table>'+
							 '</td></tr></table>';
					break;
		...
		doClick: function (id, param, item_index, event) {
			if (window.getSelection) window.getSelection().removeAllRanges(); // clear selection 
			var it = this.get(id);
			if (it && !it.disabled) {
				// event before
				var eventData = this.trigger({ phase: 'before', type: 'click', target: (typeof id != 'undefined' ? id : this.name), item: this.get(id), param: param, 
					  subItem: (typeof item_index != 'undefined' && this.get(id) ? this.get(id).items[item_index] : null), event: event });	
				if (eventData.stop === true) return false;
